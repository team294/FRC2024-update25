...
public class DriveWithController extends Command {
  ...
  public void execute() {
    ...
    fwdVelocity = (Math.abs(fwdVelocity) < OIConstants.xboxJoystickDeadband) ? 0 : scaleJoystick(fwdVelocity) * SwerveConstants.kMaxSpeedMetersPerSecond / 1.5;
    leftVelocity = (Math.abs(leftVelocity) < OIConstants.xboxJoystickDeadband) ? 0 : scaleJoystick(leftVelocity) * SwerveConstants.kMaxSpeedMetersPerSecond / 1.5;
    turnRate = (Math.abs(turnRate) < OIConstants.xboxJoystickDeadband) ? 0 : scaleTurn(turnRate) * SwerveConstants.kMaxTurningRadiansPerSecond / 1.5;
